    ...
    for i in range(len(interp_times)-1):
        if interp_demands[i] == interp_demands[i+1]:
            ...
            if insertion_mode == "Poisson":
                ...
                flowElement.set("vehsPerHour", interp_demands[i])
        ...
        for j in range(len(times)-1):
            ...
            if insertion_mode == "Poisson":
                ...
                flowElement.set("vehsPerHour", demand_flow)
            flowElement.set("departLane", depart_lane)
            flowElement.set("departSpeed", depart_speed)


def generate_route_file(route_demands_params: List[dict], sim_begin: float, sim_step: float, sim_end: float, route_filepath: str) -> None:
    ...
    for d in route_demands_params:
        generate_demand(
            d["route_id"], d["flow_id"], d["vtype_id"], d["interp_times"], d["interp_demands"], d["depart_lane"], d["depart_speed"], sim_begin, sim_step, sim_end, t_root, d["insertion_mode"]
        )
...
if __name__ == "__main__":
    ...
    route_demands = [
        {
            "route_id": "r_O2_D1", "flow_id": "f_0", "vtype_id": "car", "depart_lane": "free", "depart_speed": "max",
            "interp_times": [0, 540, 1260, 1800, 9000], "interp_demands": [500, 1500, 1500, 500, 500], "insertion_mode": "Poisson"
        },
        {
            "route_id": "r_O1_D1", "flow_id": "flow1", "vtype_id": "car", "depart_lane": "L1c_0", "depart_speed": "max",
            "interp_times": [0, 7200, 8100, 9000], "interp_demands": [3500, 3500, 1000, 1000], "insertion_mode": "Poisson"
        },
        {
            "route_id": "r_O1_D1", "flow_id": "flow2", "vtype_id": "car", "depart_lane": "L1c_1", "depart_speed": "20.0",
            "interp_times": [0, 7200, 8100, 9000], "interp_demands": [3500, 3500, 1000, 1000],
